...
class AStarGhost(Ghost):
    ...
    def getWeight(self, node, v, goalNode):
        ...
        #    f.write(str(weight) + "\n")
        #return weight
        return sum(Vector2.asInt(node.position-v.position)) + self.getDistance(node, goalNode) #generate random weights - keep track of weights in log file
                
        
    def getDistance(self, node, v):
        vec = node.position - self.goal.position
    ...
    def update(self, dt):	
        ...
        self.aStar(self.node, self.goal)
